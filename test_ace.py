...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--encoder_path",
        type=Path,
        default=Path(__file__).parent / "superpoint_v1.pth",
    ...
    parser.add_argument(
        "--image_resolution", type=int, default=1440, help="base image resolution"
    )
    ...
    annotation_output_dir = (
        base_annotation_output_dir / f"{datetime.now().strftime('%m_%d_%y_%H_%M_%S')}"
    )
    ...
    with torch.no_grad():
        for (
            ...
            for frame_idx, (
                ...
                kpt_mask = torch.zeros(
                    (scene_coordinates_3HW.shape[1], scene_coordinates_3HW.shape[2]),
                    dtype=torch.int,
                )
                ...
                kpts = sift.detect(
                    (image_B1HW.cpu().numpy().squeeze() * 255).astype(np.uint8), None
                )
                ...
                inlier_map = torch.zeros(
                    (scene_coordinates_B3HW.shape[2], scene_coordinates_B3HW.shape[3])
                )
                ...
                downsampling_rate = (
                    annotated_image.shape[0] / scene_coordinates_3HW.shape[1]
                )
                ...
                for y in range(inlier_map.shape[0]):
                    for x in range(inlier_map.shape[1]):
                        if inlier_map[y][x] != 0:
                            circle_cen_x = int(
                                x * downsampling_rate + downsampling_rate // 2
                            )
                            circle_cen_y = int(
                                y * downsampling_rate + downsampling_rate // 2
                            )
                            cv2.circle(
                                annotated_image,
                                (circle_cen_x, circle_cen_y),
                ...
                output_img_fp = str(
                    annotation_output_dir / frame_path.split("/")[-1]
                ).replace(".jpg", ".png")
    ...
    _logger.info(
        "{} frames included out of {} total frames".format(
            frames_included, len(testset)
        )
    )
